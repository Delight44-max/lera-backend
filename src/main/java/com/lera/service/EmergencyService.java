...
public class EmergencyService {
    ...
    public EmergencyDto createEmergency(String citizenId, CreateEmergencyRequest req) {
        ...
        emergencyRepo.findFirstByCitizenIdAndStatusIn(citizenId, ACTIVE_STATUSES)
                .ifPresent(e -> {
                    throw AppException.conflict("You already have an active emergency");
                });
    ...
    public EmergencyDto acceptEmergency(String emergencyId, String responderId) {
        ...
        eventPublisher.publishEvent(
                new EmergencyStatusChangedEvent(this, emergency)
        );
    ...
    public EmergencyDto getById(String emergencyId, String requestingUserId) {
        ...
        String citizenId = emergency.getCitizen() != null
                ? emergency.getCitizen().getId()
                : null;

        String responderId = emergency.getResponder() != null
                ? emergency.getResponder().getId()
                : null;

        boolean isCitizen = requestingUserId.equals(citizenId);
        boolean isAssignedResponder = requestingUserId.equals(responderId);
        boolean isPending = emergency.getStatus() == EmergencyStatus.pending;
        boolean isDispatched = emergency.getStatus() == EmergencyStatus.dispatched;


        // ✅ FIX: allow viewing BEFORE assignment if pending
        if (isCitizen || isAssignedResponder || isPending) {
            return EmergencyDto.from(emergency);
        }
