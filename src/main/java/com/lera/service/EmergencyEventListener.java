...
public class EmergencyEventListener {
    ...
    public void onEmergencyCreated(EmergencyCreatedEvent event) {
        ...
        socketIOService.sendEmergencyUpdate(emergency.getCitizen().getId(), dto);
        ...
        for (ResponderProfile profile : online) {
            notificationService.createAndSend(
                ...
                emergency,
                "🚨 New Emergency Alert",
                capitalize(emergency.getType().name()) + " reported" +
    ...
    @Async
    @EventListener
    public void onEmergencyAccepted(EmergencyAcceptedEvent event) {
        Emergency emergency = event.getEmergency();
        EmergencyDto dto = EmergencyDto.from(emergency);
        log.info("[EVENT] EmergencyAccepted → {}", emergency.getId());


        socketIOService.sendEmergencyUpdate(emergency.getCitizen().getId(), dto);


        socketIOService.sendEmergencyUpdate(emergency.getResponder().getId(), dto);
        ...
        notificationService.createAndSend(
            ...
            emergency,
            "Emergency Accepted",
            "You accepted a " + emergency.getType() + " emergency. Navigate to the incident."
        );
    }
